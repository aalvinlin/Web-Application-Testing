...
test("Foul count is incremented by 1", () => {
    ...
    gameStatus = { strikes: 1, balls: 0, fouls: 1, hits: 4 }
    expect(gameStatus.hits.toBe(2));

    gameStatus = { strikes: 0, balls: 0, fouls: 0, hits: 22 }
    expect(gameStatus.hits.toBe(1));

    gameStatus = { strikes: 1, balls: 0, fouls: 33, hits: 17 }
    expect(gameStatus.hits.toBe(34));

    gameStatus = { strikes: 0, balls: 0, fouls: 999, hits: 500 }
...
test("Strike count is incremented by 1 if strikes is below 2", () => {
    ...
    gameStatus = { strikes: 2, balls: 1, fouls: 2, hits: 165 }
    expect(gameStatus.fouls.toBe(2));

    gameStatus = { strikes: 18, balls: 19, fouls: 1, hits: 3 }
    expect(gameStatus.fouls.toBe(18));

    gameStatus = { strikes: 0, balls: 8, fouls: 13, hits: 44 }
    expect(gameStatus.fouls.toBe(1));

    gameStatus = { strikes: 45, balls: 3, fouls: 37, hits: 4 }
    expect(gameStatus.fouls.toBe(45));

    gameStatus = { strikes: 61, balls: 91, fouls: 5, hits: 9 }
